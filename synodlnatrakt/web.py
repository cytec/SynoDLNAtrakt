...
def index():
    content = db.session.query(
        db.TVShows).filter(db.TVEpisodes.show_id == db.TVShows.tvdb_id).order_by(db.TVEpisodes.added.desc()).limit(10)
...
def index(tvdb_id):
    ...
    content = db.session.query(db.TVEpisodes).filter(
        db.TVEpisodes.show_id == tvdb_id).order_by(db.TVEpisodes.season.desc()).order_by(db.TVEpisodes.episode.desc())
...
def index(mypage=1):
    ...
    pagination = Pagination(mypage, config.page_limit, count)
    content = db.session.query(
        db.Movies).filter(db.Movies.progress == 0).order_by(db.Movies.name.asc()).offset(myoffset).limit(config.page_limit)
...
def index(mypage=1):
    ...
    pagination = Pagination(mypage, config.page_limit, count)
    content = db.session.query(
        db.Movies).filter(db.Movies.rating == 0).order_by(db.Movies.name.asc()).offset(myoffset).limit(config.page_limit)
...
def index(mypage=1):
    ...
    count = db.session.query(
        db.Movies).filter(db.Movies.progress > config.min_progress).order_by(db.Movies.lastseen.desc()).count()
    pagination = Pagination(mypage, config.page_limit, count)
    content = db.session.query(db.Movies).filter(
        db.Movies.progress > config.min_progress).order_by(db.Movies.lastseen.desc()).offset(myoffset).limit(config.page_limit)
...
def index():
    ...
    if whattosearch == "episodes":
        result = db.session.query(db.TVEpisodes).filter(db.or_(db.TVEpisodes.name.like(
            u'%{0}%'.format(searchterm)), db.TVEpisodes.description.like(u'%{0}%'.format(searchterm)))).all()
        mytemplate = "list"
    if whattosearch == "movies":
        result = db.session.query(db.Movies).filter(db.or_(
            db.Movies.name.like(u'%{0}%'.format(searchterm)), db.Movies.description.like(u'%{0}%'.format(searchterm)))).all()
...
def scrobble():
    ...
    if m:
        ...
        m.scrobbled = 1
        m.progress = 100
        m.lastseen = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        m.to_database()
        return answer
    else:
        return False
...
def index():
    ...
    for cur_notification in ui.notifications.get_notifications(myip):
        messages['notification-' + str(cur_notification_num)] = {'title': cur_notification.title, 'message':
                                                                 cur_notification.message, 'type': cur_notification.type}
        cur_notification_num += 1
